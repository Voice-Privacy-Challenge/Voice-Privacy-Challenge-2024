...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    ...
    if [ ! -e "${db_root}"/LibriTTS/.complete ]; then
        for part in dev-clean dev-other test-clean test-other train-clean-100 train-other-500; do
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for name in dev-clean dev-other test-clean test-other train-clean-100 train-other-500; do
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    utils/combine_data.sh data/train-600 data/train-clean-100 data/train-other-500
fi
